...
class Solver(nn.Module):
    ...
    def train(self, loaders):
        ...
        for i in range(args.resume_iter, args.total_iters):
            ...
            g_loss, g_losses_latent = compute_g_loss(
                nets, args, x_real, y_trg, z_trgs=[z_trg, z_trg2])
            self._reset_grad()
            g_loss.backward()
            ...
            g_loss, g_losses_ref = compute_g_loss(
                nets, args, x_real, y_trg, x_refs=[x_ref, x_ref2])
            self._reset_grad()
            g_loss.backward()
            optims.generator.step()
    ...
    def sample(self, loaders):
        ...
        print('Working on {}...'.format(fname))
        utils.video_ref(nets_ema, src.x, ref.x, ref.y, fname)
...
def compute_g_loss(nets, args, x_real, y_trg, z_trgs=None, x_refs=None,):
